...
bool parseInputParameters(int ac, char* av[])
{
    ...
    try {
        ...
        desc.add_options()
            ("help,h", "searchGeck - Search gecko for the given geck ID.")
        ...
        if (vm.count("help")) {
            std::cout << "Usage: searchGeck  -i geck_id\n";
            std::cout << desc;
            return false;
        }
        ...
        if (geckId.empty())
        {
            optionString = Sbtools::Resources<std::string>::Gecko;
        }
        else 
        {
            optionString = Sbtools::Resources<std::string>::Gecko + "view?Record=" + geckId;
        }
